...
| **Ngắt mạch (circuit breaker)**: retry chỉ hợp với trục trặc chớp nhoáng; nhà cung cấp sập vài phút thì retry thành phản tác dụng (mỗi request cố 3 lần × chờ tới 8s, giữ rịt luồng, còn dội thêm tải vào dịch vụ đang ốm). Hỏng 5 lần liên tiếp → mở mạch, từ chối ngay 30s, rồi cho **một** lượt đi dò. Chỉ đếm lỗi hạ tầng (5xx/mạng chết) — **không** đếm 401/403/404, nếu không thì vài người nhập sai token đủ cắt dịch vụ của tất cả. Hai mạch riêng cho thư và AI | `app/core/breaker.py` | `pytest tests/test_breaker.py -v` (8 test); đo thật: 50 lượt bị chặn trong **0.3ms** thay vì gọi ra rồi chờ timeout |
| **Thu thập lỗi cắm-rút**: `/metrics` cho biết hệ thống có nghẽn không, nhưng không cho biết **ai gặp lỗi gì**. Có `SENTRY_DSN` → gửi kèm ngữ cảnh; không có → ghi log nội bộ (0 phụ thuộc). Bộ lọc che sạch token/tiêu đề/thân thư trước khi rời máy chủ | `app/core/errors.py` | đo thật: `Authorization`/`subject`/`body` → `[da-che]`, giữ `folder`/`user_id` |
| **Cô lập dữ liệu giữa người dùng**: lỗi tệ nhất một app email có thể mắc — người này đọc thư người kia. Không làm app sập, không hiện trong log, sống nhiều tháng trước khi thành sự cố lộ dữ liệu. Canh ở hai tầng: truy vấn (repo có lọc theo người không) và mã nguồn (hàm đọc nào quên nhận `user_id`) | `tests/test_isolation.py` | `pytest tests/test_isolation.py -v` (4 test) |
| **Sao lưu + KIỂM CHỨNG KHÔI PHỤC**: bản sao lưu chưa từng khôi phục thử thì coi như chưa có. Lệnh `verify` khôi phục vào database tạm rồi đối chiếu số dòng với bản thật; giữ 14 bản gần nhất rồi tự dọn (ổ đầy thì lần sao lưu kế tiếp thất bại đúng lúc cần nhất) | `scripts/backup_db.py` | `python scripts/backup_db.py verify` |
| **Tắt máy êm**: mỗi lần triển khai là một lần tắt. Huỷ vòng dọn nền (vòng lặp vô hạn — không huỷ thì tiến trình treo mãi) và trả kết nối database (không trả thì Postgres giữ kết nối "ma", triển khai vài lần là cạn slot) | `app/api/app.py` (shutdown) | `pytest tests/test_ops_endpoints.py::test_tat_may_khong_bi_treo -v` |
| **Di trú database (Alembic)**: `create_all` chỉ TẠO bảng mới, KHÔNG sửa bảng đã có → thêm cột vào bảng đang chạy là im lặng bỏ qua (chính lý do phải né bằng bảng riêng `session_providers`). Nay mọi thay đổi schema đi qua bản di trú chạy tiến/lùi được; database sẵn có được `stamp` sang baseline nên **giữ nguyên toàn bộ dữ liệu**; đặt `AUTO_CREATE_TABLES=false` ở môi trường thật để schema chỉ đổi khi có người chủ động chạy di trú | `alembic/`, `alembic/README` | `pytest tests/test_migrations.py -v` (3 test, gồm test dựng lại schema từ database trống) |
