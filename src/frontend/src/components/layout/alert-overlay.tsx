...
/* Giữ tối đa bấy nhiêu id đã báo. Không có trần thì danh sách phình mãi trong
   localStorage — vài tháng dùng thật là vài nghìn id, và mỗi lần đọc/ghi đều tốn.
   Cắt phần CŨ NHẤT: thư cũ thì cũng chẳng còn khả năng quay lại làm ứng viên. */
const TRAN_NHO = 500

/** Thông báo ở trên cùng tự tắt sau bấy nhiêu giây, có thanh chạy đếm ngược. */
const GIAY_HIEN = 4
...
/* ── VÌ SAO localStorage, KHÔNG PHẢI sessionStorage ──
   sessionStorage chết theo tab. Đóng trình duyệt rồi mở lại là mọi thứ được báo
   LẠI TỪ ĐẦU — kể cả những thư đã đọc từ hôm qua. Người dùng yêu cầu đúng điều
   ngược lại: mỗi thông báo chỉ hiện ĐÚNG MỘT LẦN, tính từ lúc nó xuất hiện cho
   tới những lần quay lại sau. Đó là localStorage. */
function docDaBao(): Set<string> {
  try {
    return new Set(JSON.parse(localStorage.getItem(KHOA_DA_BAO) ?? '[]') as string[])
  } catch {
    return new Set()
  }
}
function ghiDaBao(s: Set<string>) {
  try {
    const ds = [...s]
    localStorage.setItem(KHOA_DA_BAO, JSON.stringify(ds.slice(-TRAN_NHO)))
  } catch {
...
export function AlertOverlay({ emails }: { emails: Email[] }) {
  ...
  /* CHỈ XÉT HỘP THƯ ĐẾN.
     `emails` là thư của THƯ MỤC ĐANG XEM. Bấm sang "Thư rác" là cả danh sách đổi
     thành một tập id chưa từng thấy, nên mọi lá đều bị coi là "thư mới" và dội một
     loạt thông báo — đúng triệu chứng người dùng báo. Cùng lý do đó, hạn trích từ
     thư rác cũng không nên báo: thứ nằm trong thư rác không phải việc phải làm. */
  const hopThuDen = useMemo(
    () => emails.filter((e) => (e.folder ?? 'inbox') === 'inbox'),
    [emails],
  )
  ...
  const ungVien = useMemo<Tin[]>(() => {
    ...
    for (const c of trichCamKet(hopThuDen)) {
    ...
    if (_daThay === null) {
      _daThay = new Set(hopThuDen.map((e) => e.id))
    } else {
      for (const e of hopThuDen) {
  ...
  useEffect(() => {
    ...
    /* CẢ HAI LOẠI đều tự tắt sau 4 giây, và có THANH CHẠY cho thấy còn bao lâu.
       Bản trước để hạn ở lại vĩnh viễn cho tới khi bấm đóng — lập luận là "đó là
       thứ họ cần thấy". Nhưng một tấm chắn không tự đi thì người dùng học được
       cách phớt lờ nó, và nó thành đúng thứ nó cố tránh: nhiễu.
       Hạn không mất đi đâu cả — nó vẫn nằm ở trang Lịch trình, chỗ dành cho nó. */
    const hens = moi.map((t) =>
      window.setTimeout(() => setHien((cu) => cu.filter((x) => x.id !== t.id)), GIAY_HIEN * 1000),
    )
    ...
    >
      {hien.map((t) => (
        <div
          key={t.id}
          className={cn(
            // `relative` để thanh chạy bám mép dưới thẻ. `overflow-hidden` để nó
            // không tràn ra ngoài góc cắt của `.goc-cat`.
            'nhay-bat goc-cat pointer-events-auto relative overflow-hidden',
            'flex w-[min(560px,94vw)] items-center gap-3',
        ...
        >
          ...
          {/* THANH CHẠY — cho thấy thông báo còn ở lại bao lâu.
              Không có nó thì thông báo biến mất đột ngột và người dùng không biết
              mình còn bao nhiêu thời gian để đọc; có nó thì họ tự quyết định đọc
              nhanh hay bấm giữ lại. `scaleX` (không phải `width`) để chạy trên
              GPU, không bắt trình duyệt tính lại bố cục 60 lần mỗi giây. */}
          <span
            aria-hidden
            className="pointer-events-none absolute inset-x-0 bottom-0 h-[2px] origin-left"
            style={{
              background: 'var(--rr)',
              animation: `toast-bar ${GIAY_HIEN}s linear forwards`,
            }}
          />
        </div>
      ))}
