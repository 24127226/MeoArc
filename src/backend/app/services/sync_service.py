...
def _apply_gmail_changes(db: Session, user_id: int, token: str, changes: dict) -> int:
    """added/updated → fetch full rồi upsert; deleted → chuyển thư mục 'trash' trong DB."""
    n = 0
    for mid in changes.get("added", []) + changes.get("updated", []):
        try:
            em = gmail_service.get_message(token, mid)
        except Exception as exc:
            logger.warning("history: bỏ qua %s (%s)", mid, exc)
            continue
        email_store_repo.upsert(db, user_id, "google", em, folder=em.folder or "inbox",
...
def incremental_sync(db: Session, user_id: int, provider: str, token: str) -> int:
    ...
    if provider == "google":
        ...
        try:
            ...
            return initial_sync(db, user_id, provider, token)
        n = _apply_gmail_changes(db, user_id, token, changes)
