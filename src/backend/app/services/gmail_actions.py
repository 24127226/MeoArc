...
def list_label_names(access_token: str) -> list[str]:
    """(Tích hợp) Trả TÊN mọi nhãn trong hộp thư — cho agent tool `list_labels`.
    CỘNG THÊM, không đổi hành vi cũ; chỉ ĐỌC (không ghi nên khỏi dọn cache)."""
    headers = {"Authorization": f"Bearer {access_token}"}
    with httpx.Client(timeout=15) as client:
        ...
        if r.status_code == 403:
            raise GmailPermissionError()
        r.raise_for_status()
        return [lb.get("name", "") for lb in r.json().get("labels", []) if lb.get("name")]
