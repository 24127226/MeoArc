...
class ToolRegistry:
    ...
    def to_langchain_tools(
        ...
        for spec in self._tools.values():
            # (Tích hợp - SỬA BUG) Bỏ tool nếu category NẰM TRONG danh sách loại trừ.
            # Bản gốc develop viết `not in` → lọc NGƯỢC (chỉ giữ tool BỊ loại) khiến LLM
            # gần như không thấy tool nào. Sửa `in` để giữ ĐÚNG các tool KHÔNG bị exclude.
